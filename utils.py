...
def tran_pixID_to_robotXYZ(pred_pix_id, depth_value):
...
WORK_SPACE_LIMIT = np.asarray([[-0.9, -0.2], [-0.35, 0.35], [0.0001, 0.2]])
...
def load_depth_mask_heightmap_data(state_path):
    ...
    # # original depth data
    # depth_arr = origin_depth_arr
    # depth_arr = (depth_arr - _configs.MIN_DEPTH_ARR) / (_configs.MAX_DEPTH_ARR - _configs.MIN_DEPTH_ARR)
    # depth_arr = np.clip(depth_arr,0,100)
    # mask
    ...
    # ##### check if the depth-mask is correct by visualization #####
    # res_img_save_path = 'depth_mask_visualization/'
    # cv2.imwrite(res_img_save_path + state_path.replace('state_depth/','').replace('npy','png'), mask_arr*255)

    return np.stack((mask_arr, mask_arr, mask_arr))

def trans_to_heightmap(npy_path,depth_type):
...
def trans_label_arr_to_label_id(label_pos_arr):
    pos_x = label_pos_arr[0]
    pos_y = label_pos_arr[1]
    pix_x = round((abs(pos_x)-abs(WORK_SPACE_LIMIT[0][1]))/0.7 * 256)
    pix_y = round((pos_y - WORK_SPACE_LIMIT[1][0]) / 0.7 * 256)
    arr = np.array(list(range(256*256))).reshape((256,256))
    ### x-w, y-h ###
    return arr[int(pix_x)][int(pix_y)]
def trans_label_arr_to_label_pix(label_pos_arr):
    pos_x = label_pos_arr[0]
    pos_y = label_pos_arr[1]
    pix_x = round((abs(pos_x)-abs(WORK_SPACE_LIMIT[0][1]))/0.7 * 512)
    pix_y = round((pos_y - WORK_SPACE_LIMIT[1][0]) / 0.7 * 512)
    return int(pix_x), int(pix_y)
def trans_actId_to_robotPos(act_id):
    pix_x = int(act_id%256)
    pix_y = int(act_id/256)
    x_resolution = (WORK_SPACE_LIMIT[0][0] - WORK_SPACE_LIMIT[0][1]) / 256
    y_resolution = (WORK_SPACE_LIMIT[1][1] - WORK_SPACE_LIMIT[1][0]) / 256
    pos_x = WORK_SPACE_LIMIT[0][1] + pix_x*x_resolution
    pos_y = WORK_SPACE_LIMIT[1][0] + pix_y*y_resolution

    return pos_x, pos_y
def get_size_from_depth(depth_name):
    dep_arr = cv2.imread(depth_name,0)
    dep_arr = np.clip(dep_arr * 10000, 0, 255)
    # dep_arr = dep_arr.astype(np.uint8)

    contours, hierarchy = cv2.findContours(dep_arr.astype(np.uint8),cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)  # CHAIN_APPROX_NONE,CHAIN_APPROX_SIMPLE
    w_x = 0
    h_y = 0
    for cnt in contours:
        ...
        if w >= 3 and h >= 3:
            ...
            h_y = h
    z_margin = 5
    return round(w_x * 30 / 128), round(h_y * 30 / 128), z_margin